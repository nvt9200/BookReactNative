...
const Category = ({ navigation }) => {
	...
	useEffect(() => {
		axios
			.get('http://myebookapp.000webhostapp.com//api.php?cat_list')
			.then((res) => {
				setCategories(res.data.EBOOK_APP);
				if (
					categories[0].cat_image == null ||
					categories[0].cat_image == undefined ||
					categories[0].cat_image == ''
				) {
			...
			<View style={{ backgroundColor: COLORS.black }}>
				...
				>
					<View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
						<Text style={{ ...FONTS.h1, color: COLORS.white }}>Thể loại</Text>
					</View>
				</View>
				<View style={{ marginTop: SIZES.padding, flexDirection: 'row', flexWrap: 'wrap' }}>
					<FlatList
						data={categories}
